...
Output layer와 그 직전 hidden layer 사이의 weight (즉 $W_{kl}$ matrix의 원소들, 예를 들면 $w_{k_1l_1}$)가 error에 미치는 영향은 $\frac{\partial{E_1}}{\partial{w_{k_1l_1}}}$ 로 계산할 수 있는데, 그림에서 보듯이 크게 세 단계를 거치는 chain rule로 표시할 수 있다.  
