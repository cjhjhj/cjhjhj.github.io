...
첫번째, $h2_{out_1}$는 $h2_{in_1}$를 input으로 갖는 sigmoid 함수로 표현되고, 그 미분식은 위에서 미리 유도되었다.
...
Weight $w_{j\cdot k_1}$는 $h2_{in_1}$, $w_{j\cdot k_2}$는 $h2_{in_2}$, $w_{j\cdot k_3}$은 $h2_{in_3}$에만 기여하므로 matrix operation으로 표현하면,  
$$\begin{bmatrix} 
